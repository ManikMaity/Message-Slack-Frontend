...
function Editor({
  ...
  async function handlePromptSubmit(e) {
    ...
    if (response && quillRef.current) {
      ...
      quill.setSelection(range.index + response.length);
  ...
  function handleSend() {
    ...
    if (!simpleText.trim() && !imageUrlRef.current) return;
  ...
  useEffect(() => {
    ...
    if (defaultValueRef.current) {
      editor.clipboard.dangerouslyPasteHTML(defaultValueRef.current);
    ...
    <div className="flex flex-col md:p-1 relative">
      {imageFile && (
        <div className="h-20 w-20 rounded-md overflow-hidden shadow-md absolute z-20 border-2 mb-2 -top-20 bg-gray-200 dark:bg-gray-800">
          ...
          {imageUrl && (
            <img className="size-full" src={imageUrl} alt="uploaded" />
          )}
        </div>
      )}
      ...
      <div className="flex flex-col md:border border-slate-300 dark:bg-slate-950 bg-gray-200 rounded-md overflow-hidden focus-within:shadow-sm focus-within:border-slate-400">
        {aiResponseOpen && (
          <form
            className="w-full h-10 bg-slate-950 p-0.5 flex justify-between"
          ...
          <div className="flex gap-1">
            ...
            <CustomTooltip content="Generate AI text" side="left">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setAiResponseOpen(!aiResponseOpen)}
              >
                <Bot />
              </Button>
            </CustomTooltip>
